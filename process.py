...
Lav en sektion per agent/subdomain. Bevar ALT relevant indhold.
"""
...
def fetch_transcript(video_id):
    ...
    try:
        ...
        t = transcript_list.find_generated_transcript([tr.language_code for tr in transcript_list])
    entries = t.fetch()
    return " ".join(getattr(e, "text", e.get("text", "") if isinstance(e, dict) else "") for e in entries)
...
def analyze(transcript, speaker, url):
    ...
    chunks = []
    with client.messages.stream(model="claude-opus-4-7", max_tokens=16000, messages=[{"role": "user", "content": prompt}]) as stream:
...
def parse_and_save(analysis, speaker_slug):
    ...
    for section in analysis.split("===END==="):
        ...
        out_path.write_text(body[1].strip(), encoding="utf-8")
...
def process(speaker, url):
    ...
    if not vid:
        ...
        return []
    print(f"[1/3] Henter transskript...")
    transcript = fetch_transcript(vid)
    print(f"[2/3] Analyserer ({len(transcript)} tegn)...")
    analysis = analyze(transcript, speaker, url)
    print(f"[3/3] Gemmer...")
    saved = parse_and_save(analysis, slugify(speaker))
    if not saved:
        debug = DRIVE_BASE / f"_debug_{date.today().isoformat()}_{slugify(speaker)}_{vid}.txt"
        debug.write_text(analysis, encoding="utf-8")
        print(f"  Ingen filer parset. Debug gemt: {debug}")
    for p in saved:
        print(f"  Gemt: {p}")
    return saved
